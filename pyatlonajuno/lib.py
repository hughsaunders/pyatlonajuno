...
class Juno451:
    ...
    def getPowerState(self):
        """returns on or off"""
        if requests.get(self.url+"/aj.html?a=sys").json()['PW'] == 1:
            return "on"
        else:
            return "off"
    ...
    def setPowerState(self, state):
        if state == "on":
            requests.get(self.url+"/aj.html?a=command&cmd=PWON")
        elif state == "off":
            requests.get(self.url+"/aj.html?a=command&cmd=PWOFF")
        else:
    ...
    def getSource(self):
        return requests.get(self.url+"/aj.html?a=avs").json()['inp']
    ...
    def setSource(self, source):
        ...
        requests.get(self.url+"/aj.html?a=command&cmd=x{}AVx1".format(source))
